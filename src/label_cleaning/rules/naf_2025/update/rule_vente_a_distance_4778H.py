...
def distance_selling_4778H_2025(
    df: pd.DataFrame, methods=None, methods_params=None
) -> pd.DataFrame:
    methods = filter_methods(methods, exclude=["regex", "similarity"])
    TEXTUAL_INPUTS_CLEANED.remove("activ_nat_lib_et_clean")
    terms = [
        "vente a distance",
    ...
    df[NACE_REV2_1_COLUMN] = np.where(match_mask, "4778H", df[NACE_REV2_1_COLUMN])
