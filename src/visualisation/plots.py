...
from constants.paths import URL_SIRENE4_NAF2025, URL_OUTPUT_NAF2025, URL_REPORT_NAF2025
from constants.targets import NACE_REV2_1_COLUMN
from utils.io import download_data


def plot_distribution_before_after(input_path=URL_SIRENE4_NAF2025, output_path=URL_OUTPUT_NAF2025):
    """
...
def plot_rule_report_comparison(base_report_path=URL_REPORT_NAF2025, methods=("regex", "fuzzy")):
    """
    ...
    fig_code = px.bar(
        ...
        x="APE_AFTER", y="count", color="method", barmode="group",
    ...
    fig_rule = px.bar(
        df_all.groupby(["_log_rules_applied", "method"]).size().reset_index(name="count"),
        x="_log_rules_applied", y="count", color="method", barmode="group",
        title="Number of modified rows per rule and method"
    )
    fig_rule.show()


def plot_methods_overlap_0(base_report_path=URL_REPORT_NAF2025,
                           methods=("regex", "fuzzy"),
